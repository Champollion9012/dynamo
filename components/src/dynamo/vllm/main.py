...
async def worker(argv: list[str] | None = None) -> None:
    ...
    if snapshot_controller is not None:
        # The flock is held by the open fd, not by any Python reference: the raw
        # fd is never closed by garbage collection, and the kernel releases it
        # when the process exits. Nothing needs to retain the handle.
