...
async def init_omni_stage(
    ...
    engine = _create_engine(
        config.model, my_config, stage_type, stage_id, trust_remote_code
    )
...
def _register_single_stage_pipeline(
    model: str, stage_id: int, trust_remote_code: bool
) -> Iterator[str]:
...
def _create_engine(
    ...
    devices = (stage_arg.get("runtime") or {}).get("devices")
    if devices is not None:
        stage_entry["devices"] = devices
    ...
    with _register_single_stage_pipeline(
        model, stage_id, trust_remote_code
    ) as pipeline_key:
        deploy_config = {
