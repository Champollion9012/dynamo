...
impl EppRouter {
    ...
    pub async fn from_selector(cfg: EppStandaloneConfig) -> Result<Self> {
        let selectors = Self::build_selectors(&cfg).await?;
    ...
    async fn build_selectors(cfg: &EppStandaloneConfig) -> Result<RoleSelectors> {
        if !cfg.topology_mode.is_disaggregated() {
            return Ok(RoleSelectors::Aggregated(Arc::new(
                Selector::new(cfg).await?,
            )));
        }
    ...
    pub async fn from_selection_service(
        ...
        // A single prebuilt service cannot express a role split, and extending
        // this entry point to a pair is a public-API decision #13407 owns.
        anyhow::ensure!(
            !cfg.topology_mode.is_disaggregated(),
            "EppRouter::from_selection_service does not support \
             DYN_EPP_TOPOLOGY_MODE=disaggregated: a role split needs one SelectionService per \
             role; use the stock EPP binary"
        );
        let selectors =
            RoleSelectors::Aggregated(Arc::new(Selector::from_service(&cfg, service).await?));
        let (renderer, reflector, reflector_ready) = Self::dependencies(&cfg).await?;
        Ok(Self::from_selector_parts(
            cfg,
            renderer,
    ...
    fn from_selector_parts(
        ...
        let peer_ready = selectors.peer_ready();
...
mod tests {
    ...
    async fn router_over(cfg: EppStandaloneConfig, workers: Vec<RawWorker>) -> EppRouter {
        let selectors = RoleSelectors::Aggregated(Arc::new(
            Selector::new(&cfg).await.expect("selector should build"),
        ));
        let renderer = VllmRenderClient::new(
            &cfg.tokenizer_service_url,
            Duration::from_millis(cfg.tokenization_timeout_ms),
