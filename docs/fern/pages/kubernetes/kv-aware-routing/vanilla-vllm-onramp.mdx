...
`replica-agg` Service port. `DYN_EPP_PEER_SERVICE` is rejected under
`DYN_EPP_TOPOLOGY_MODE=disaggregated`: both role selectors would resolve the same `replica-agg`
port and race to bind it.
