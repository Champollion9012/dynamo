...
async fn collect_worker_pools(
    ...
    for descriptor in catalog.pools() {
        ...
        let Some(domain) = membership.domain.as_ref() else {
            continue;
        };
        ...
        result.push(WorkerPool {
            ...
            role,
            block_size_tokens: domain.query_semantics.kv_block_size(),
            expected_ranks,
            observed_ranks,
            live_workers: lifecycle_active.then_some(membership.runtime_configs.len() as u64),
            capacity_blocks,
            used_blocks,
