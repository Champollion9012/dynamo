    ...
    Sel: WorkerSelector<ModelRuntimeConfig> + Send + 'static,
{
    async fn select_request_outcome(
        ...
        phase: RequestPhase,
        is_query_only: bool,
        session_target: Option<AffinityTarget>,
    ...
    pub(crate) async fn dispatch_kv_plan(
            ...
            .await
        {
            Ok(guard) => guard,
            Err(error) => {
                self.invalidate_dead_hard_affinity(&mut affinity, selected_target, &error);
                return Err(error);
            }
        };
        ...
        match affinity {
            Some(affinity) => self.track_session_affinity(affinity, selected_target, stream),
            None => Ok(stream),
        }
    }
