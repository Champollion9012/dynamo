...
pub struct WorkerLoadState {
    ...
    kv_used_source_observed_at_unix_ms: HashMap<u32, u64>,
...
impl WorkerLoadState {
    fn clear_observations(&mut self) {
        ...
        self.kv_used_source_observed_at_unix_ms.clear();
    ...
    fn update_from_active_load_at(
        ...
        if let Some(kv_used_blocks) = active_load.kv_used_blocks {
            ...
            self.kv_used_source_observed_at_unix_ms
                .insert(dp_rank, source_observed_at_unix_ms);
        }
...
fn pool_snapshot(
    ...
    let mut source_observed_at_unix_ms: Option<u64> = None;
    ...
    for worker_id in workers {
        ...
        for &dp_rank in &state.expected_dp_ranks {
            ...
            let Some(&source_observed_at) = state
                .kv_used_source_observed_at_unix_ms
                .get(&dp_rank)
            ...
            source_observed_at_unix_ms = Some(
                source_observed_at_unix_ms.map_or(source_observed_at, |current| {
                    current.min(source_observed_at)
                }),
            );
    ...
    KvWorkerPoolSnapshot {
        ...
        active_decode_blocks,
        active_prefill_tokens,
        source_observed_at_unix_ms: source_observed_at_unix_ms.unwrap_or_default(),
        complete,
    }
}
...
mod tests {
    ...
    fn kv_pool_snapshot_requires_fresh_capacity_and_occupancy_for_every_rank() {
        ...
        second.kv_used_source_observed_at_unix_ms.insert(2, 5);
        ...
        assert_eq!(multiple_workers.source_observed_at_unix_ms, 5);
        states.remove(&8);
    ...
    fn clearing_worker_observations_preserves_config_but_requires_fresh_occupancy() {
        ...
        state.kv_used_source_observed_at_unix_ms.insert(0, 10);
        ...
        assert!(state.kv_used_source_observed_at_unix_ms.is_empty());
