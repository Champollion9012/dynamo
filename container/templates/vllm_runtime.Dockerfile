...
# minimal forward-port from SKT's axk2-v0.23.0 branch directly to the installed
# Python package. vLLM 0.26.0 already includes the newer upstream DSpark runtime,
# but needs the later upstream sparse-MLA-plus-SWA-draft KV allocation fix.
# Validate config conversion, registry wiring, and that cache grouping fallback
# rather than overlaying SKT's older DSpark implementation on top of it.
